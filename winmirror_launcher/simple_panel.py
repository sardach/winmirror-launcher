...
DEFAULT_LAUNCHER_SLOT_UNITS = 4
DEFAULT_TINT2_PLACEMENT = "cell"
TINT2_RESTART_COOLDOWN_MS = 650
...
MAX_LAYOUT_LIMIT = 64
EQUILATERAL_TRIANGLE_RATIO = math.sqrt(3.0) / 2.0
...
def normalize_layout_limit(value):
    ...
    if value <= 0:
        return None
    return max(1, min(MAX_LAYOUT_LIMIT, value))
...
def normalize_mirror_layout_mode(value):
    if value == "rect-grid":
        return "grid"
    return value if value in MIRROR_LAYOUT_MODES else "grid"
...
def tint2_window_name(role):
    if role == "combined":
        return "winmirror-launcher-tint2"
    return f"winmirror-launcher-tint2-{role}"


def build_tint2_config(profile, width=240, height=48, orientation="horizontal", placement="cell", role="combined"):
    ...
    role = role if role in {"combined", "tray", "launchers"} else "combined"
    launcher_lines = []
    panel_items = "S"
    ...
    if option_block:
        option_block += "\n"
    window_name = tint2_window_name(role)
...
class ClockSlot(Gtk.DrawingArea):
    ...
    def on_draw(self, widget, cr):
        ...
        font_size = self.font_size if len(lines) == 1 else max(MIN_CLOCK_FONT_SIZE, self.font_size - 2)
...
class Tint2Slot(Gtk.Box):
    ...
    def should_take_systray(self):
        return self.take_systray

    def tint2_roles(self):
        return ("tray",)

    def role_process(self, role):
        return self.processes.get(role)

    def role_is_running(self, role):
        process = self.role_process(role)
        return process is not None and process.poll() is None

    def sync_primary_process(self):
        for role in self.tint2_roles():
            process = self.role_process(role)
            if process is not None:
                self.process = process
                self.config_path = self.config_paths.get(role)
                self.window_id = self.window_ids.get(role)
                self.config_size = self.config_sizes.get(role)
                self.config_orientation = self.config_orientations.get(role)
                return
        self.process = None
        self.config_path = None
        self.window_id = None
    ...
    def start(self):
        ...
        if not self.uses_external_tint2():
            self.stop()
            return
        roles = self.tint2_roles()
        stale_roles = [role for role in self.processes if role not in roles]
        for role in stale_roles:
            self.stop_role(role)
        if all(self.role_is_running(role) for role in roles):
            return
        if subprocess.run(["/bin/sh", "-lc", "command -v tint2"], stdout=subprocess.DEVNULL, stderr=subprocess.DEVNULL).returncode != 0:
            ...
            return
        if self.should_take_systray():
            self.suspend_external_tint2_panels()
        for role in roles:
            if self.role_is_running(role):
                continue
            self.write_config(role)
            try:
                self.processes[role] = subprocess.Popen(
                    ["tint2", "-c", str(self.config_paths[role])],
    ...
    def suspend_external_tint2_panels(self):
        ...
        for line in proc.stdout.splitlines():
            ...
            try:
                ...
                continue
            command = parts[1]
            if pid in own_pids or command.startswith("pgrep ") or "/tmp/winmirror-tint2" in command:
                continue
    ...
    def write_config(self, role="combined"):
        config_path = self.config_paths.get(role)
        if config_path is None:
            handle = tempfile.NamedTemporaryFile(
                "w",
                encoding="utf-8",
                prefix=f"winmirror-tint2-{role}-",
                suffix=".tint2rc",
                delete=False,
            )
            self.config_paths[role] = Path(handle.name)
        else:
            handle = config_path.open("w", encoding="utf-8")
        with handle:
            target_rect = self.target_rects.get(role) or self.target_rect
            width = target_rect[2] if target_rect else DEFAULT_TILE_WIDTH * DEFAULT_TINT2_SLOT_UNITS
            height = target_rect[3] if target_rect else DEFAULT_TILE_HEIGHT
            self.config_sizes[role] = (width, height)
            self.config_orientations[role] = self.orientation
            placement = self.owner.tint2_placement if self.owner is not None else DEFAULT_TINT2_PLACEMENT
            handle.write(build_tint2_config(self.profile, width, height, self.orientation, placement, role))
        self.sync_primary_process()

    def restart(self):
        was_running = any(self.role_is_running(role) for role in self.processes)
        self.stop()
    ...
    def stop(self):
        if self.move_source_id is not None:
            ...
            self.move_source_id = None
        if self.restart_source_id is not None:
            GLib.source_remove(self.restart_source_id)
            self.restart_source_id = None
        for role in list(self.processes) + [role for role in self.config_paths if role not in self.processes]:
            self.stop_role(role)
        self.sync_primary_process()
    ...
    def stop_role(self, role):
        ...
        if process is not None and process.poll() is None:
            ...
            try:
                ...
                process.kill()
        if config_path_text:
            self.kill_tint2_processes_for_config(config_path_text)
        if config_path is not None:
            try:
                ...
                pass
        self.processes.pop(role, None)
        self.config_paths.pop(role, None)
        self.config_sizes.pop(role, None)
        self.config_orientations.pop(role, None)
        self.window_ids.pop(role, None)
    ...
    def split_target_rects(self, x, y, width, height):
        return {role: (int(x), int(y), width, height) for role in self.tint2_roles()}
    ...
    def set_target_rect(self, x, y, width, height):
        ...
        if self.uses_launcher_grid():
            ...
            self.target_rect = None
            self.target_rects = {}
            if self.processes or self.config_paths:
                self.stop()
            return
        else:
            self.target_rect = (int(x), int(y), width, height)
            self.target_rects = self.split_target_rects(x, y, width, height)
        size_changed = False
        for role in self.tint2_roles():
            target_rect = self.target_rects.get(role)
            config_size = self.config_sizes.get(role)
            if not self.role_is_running(role) or target_rect is None or config_size is None:
                continue
            size_changed = size_changed or (
                abs(config_size[0] - target_rect[2]) > 2
                or abs(config_size[1] - target_rect[3]) > 2
                or self.config_orientations.get(role) != self.orientation
            )
        if size_changed:
    ...
    def schedule_restart(self):
        if self.restart_source_id is not None:
            GLib.source_remove(self.restart_source_id)
        self.restart_source_id = GLib.timeout_add(TINT2_RESTART_COOLDOWN_MS, self.restart_after_cooldown)
        self.schedule_move(force=True)

    def restart_after_cooldown(self):
        self.restart_source_id = None
        self.restart()
        return False
    ...
    def move_tint2_window(self):
        roles = self.tint2_roles()
        if not any(self.role_is_running(role) and self.target_rects.get(role) for role in roles):
            self.move_source_id = None
            return False
        missing_window = False
        for role in roles:
            if not self.role_is_running(role):
                continue
            target_rect = self.target_rects.get(role)
            if target_rect is None:
                continue
            window_id = self.window_ids.get(role) or self.find_tint2_window_id(role)
    ...
    def find_tint2_window_id(self, role="combined"):
        process = self.role_process(role)
        if process is None:
            return None
        ...
        if proc.returncode == 0:
            for line in proc.stdout.splitlines():
                ...
                if len(parts) >= 3 and parts[2] == pid:
                    self.window_ids[role] = parts[0]
                    self.sync_primary_process()
                    return self.window_ids[role]
        proc = subprocess.run(["xdotool", "search", "--pid", pid], text=True, capture_output=True, check=False)
        if proc.returncode == 0:
            for line in proc.stdout.splitlines():
                window_id = line.strip()
                if window_id:
                    self.window_ids[role] = window_id
                    self.sync_primary_process()
                    return self.window_ids[role]
        proc = subprocess.run(
            ["xdotool", "search", "--name", f"^{tint2_window_name(role)}$"],
            text=True,
            capture_output=True,
            check=False,
        )
        if proc.returncode == 0:
            ...
            if matches:
                self.window_ids[role] = matches[-1]
                self.sync_primary_process()
                return self.window_ids[role]
        return None
...
class SimpleMirrorTile(Gtk.DrawingArea):
    ...
    def triangle_path(self, cr, width, height):
        if self.triangle_orientation == "blob":
            ...
            cr.line_to(width * 0.25, height)
            cr.line_to(0, height * 0.5)
        elif self.triangle_orientation == "down":
            cr.move_to(0, 0)
            cr.line_to(width, 0)
        ...
        elif self.triangle_orientation == "right":
            cr.move_to(0, 0)
            cr.line_to(width, height / 2.0)
            cr.line_to(0, height)
        elif self.triangle_orientation == "right-nw":
            cr.move_to(0, 0)
            cr.line_to(width, 0)
            cr.line_to(0, height)
        elif self.triangle_orientation == "right-se":
            cr.move_to(width, height)
            cr.line_to(width, 0)
            cr.line_to(0, height)
        elif self.triangle_orientation == "scalene-a":
            cr.move_to(0, 0)
            cr.line_to(width, 0)
            cr.line_to(width * 0.22, height)
        elif self.triangle_orientation == "scalene-b":
            cr.move_to(width, height)
            cr.line_to(0, height)
    ...
    def point_in_triangle(self, x, y, width, height):
        ...
        if self.triangle_orientation == "hex":
            if y <= height / 2.0:
                ...
                inset = (0.25 * width) * ((y - (height / 2.0)) / max(1.0, height / 2.0))
            return inset <= x <= width - inset
        if self.triangle_orientation == "right-nw":
            return (x / width) + (y / height) <= 1.0
        if self.triangle_orientation == "right-se":
            return (x / width) + (y / height) >= 1.0
        if self.triangle_orientation == "scalene-a":
            left_edge = (0.22 * width) * (y / height)
            return left_edge <= x <= width - ((width - 0.22 * width) * (y / height))
        if self.triangle_orientation == "scalene-b":
            right_edge = width - (0.22 * width) * ((height - y) / height)
            return (width - 0.78 * width) * ((height - y) / height) <= x <= right_edge
        if self.triangle_orientation in {"up", "down"}:
            half_span = (y / height) * (width / 2.0) if self.triangle_orientation == "up" else ((height - y) / height) * (width / 2.0)
            center = width / 2.0
            return center - half_span <= x <= center + half_span
        half_span = (x / width) * (height / 2.0) if self.triangle_orientation == "right" else ((width - x) / width) * (height / 2.0)
        center = height / 2.0
    ...
    def close_icon_rect(self, width, height):
        ...
        if self.triangle_orientation == "left":
            return (0, max(0, (height - size) / 2.0), size, size)
        ...
        if self.triangle_orientation == "right-se":
            return (max(0, width - size - 2), max(0, height - size - 2), size, size)
        if self.triangle_orientation == "scalene-a":
            return (max(0, width - size - 2), 2, size, size)
        if self.triangle_orientation == "scalene-b":
            return (2, max(0, height - size - 2), size, size)
    ...
    def handle_order_edit_click(self, event):
        ...
        if event.y >= height - control_size:
            if event.x <= control_size:
                self.panel.move_tile(self, -self.panel.layout_vertical_delta())
                return True
            if event.x >= width - control_size:
                self.panel.move_tile(self, self.panel.layout_vertical_delta())
                return True
...
class SimpleLauncherPanel:
    def __init__(
        ...
        self.fps = fps
        self.window_decorated = bool(window_decorated)
        self.show_title = bool(show_title)
        self.show_close = bool(show_close)
        self.show_workspace = bool(show_workspace)
        self.hover_mode = normalize_hover_mode(hover_mode, hover_expand)
        self.hover_expand = self.hover_mode != "off"
        self.hover_scale = normalize_hover_scale(hover_scale, self.hover_mode, hover_expand)
        self.mirror_layout_mode = normalize_mirror_layout_mode(mirror_layout_mode)
        self.max_layout_columns = normalize_layout_limit(max_layout_columns)
        self.max_layout_rows = normalize_layout_limit(max_layout_rows)
        self.show_borders = bool(show_borders)
        self.label_mode = normalize_label_mode(label_mode)
        ...
        self.tint2_placement = DEFAULT_TINT2_PLACEMENT
        ...
        self.win.set_decorated(self.window_decorated)
        self.configure_transparent_window()
        ...
        self.grid.put(self.launcher_grid, 0, 0)
    ...
    def configure_transparent_window(self):
        ...
        panel_state.update(
            {
                ...
                "fps": self.fps,
                "mirror_layout_mode": self.mirror_layout_mode,
                "max_layout_columns": self.max_layout_columns,
                "max_layout_rows": self.max_layout_rows,
                "window_decorated": self.window_decorated,
    ...
    def widget_slot_units(self, widget):
        ...
        if widget is self.launcher_grid:
            return self.launcher_units
        return 1
    ...
    def choose_grid_shape(self, width, height, count):
        ...
        height = max(1, int(height))
        best = None
        max_columns = min(count, self.max_layout_columns or count)
        ...
        if best is None:
            ...
            return columns, int(math.ceil(float(count) / columns))
        return best[1], best[2]

    def choose_triangle_shape(self, width, height, count):
        if count <= 1:
        ...
        for columns in range(1, max_columns + 1):
            ...
            if self.max_layout_rows is not None and rows > self.max_layout_rows:
                continue
            tile_width = width / (1.0 + max(0, columns - 1) * 0.5)
            tile_height = height / rows
            area = tile_width * tile_height * 0.5
            aspect = tile_width / max(1.0, tile_height)
            aspect_penalty = abs(math.log(max(0.1, aspect / 1.15)))
            empty = (columns * rows) - count
            score = area - (area * aspect_penalty * 0.22) - (empty * 30)
            if best is None or score > best[0]:
    ...
    def iter_triangle_layout_positions(self, entries, cell_columns, flow="horizontal"):
        ...
        for widget, units in entries:
            ...
            column = cursor % cell_columns
            if column + span_slots > cell_columns:
                cursor += cell_columns - column
                if not isinstance(widget, SimpleMirrorTile) and cursor % 2:
                    cursor += 1
    ...
    def choose_triangle_flow_shape(self, width, height, entries):
        if not entries:
            return "horizontal", 1, 1, max(1, int(width)), max(1, int(height))
        width = max(1, int(width))
        height = max(1, int(height))
        ...
        for cell_columns in range(min_cells, max_cells + 1):
            if self.max_layout_columns is not None and cell_columns > self.max_layout_columns:
                continue
        ...
        if best is None:
            ...
            rows = int(math.ceil(float(slot_count) / max(1, columns)))
    ...
    def choose_hexagon_shape(self, width, height, entries):
        if not entries:
            return 1, 1, max(1, int(width)), max(1, int(height))
        width = max(1, int(width))
        height = max(1, int(height))
        slot_count = sum(units for _widget, units in entries)
        max_columns = min(max(1, slot_count), self.max_layout_columns or max(1, slot_count))
        best = None
        for columns in range(1, max_columns + 1):
            rows = self.layout_rows_for_entries(entries, columns)
            if self.max_layout_rows is not None and rows > self.max_layout_rows:
                continue
            hex_width = width / (1.0 + max(0, columns - 1) * 0.75)
            hex_height = height / (rows + (0.5 if columns > 1 else 0.0))
            visible_area = min(hex_width, MAX_TILE_WIDTH) * min(hex_height, MAX_TILE_HEIGHT) * 0.75
            aspect = hex_width / max(1.0, hex_height)
            aspect_penalty = abs(math.log(max(0.1, aspect / 1.08)))
            empty_slots = (columns * rows) - slot_count
            score = visible_area - (visible_area * aspect_penalty * 0.14) - (empty_slots * 18)
            if height > width and columns > 2:
                score -= (columns - 2) * 28
            if best is None or score > best[0]:
                best = (score, columns, rows, hex_width, hex_height)
        if best is None:
            columns = min(max_columns, max(1, slot_count))
            rows = self.layout_rows_for_entries(entries, columns)
            return columns, rows, max(1, int(width / (1.0 + max(0, columns - 1) * 0.75))), max(1, int(height / max(1, rows)))
        return best[1], best[2], max(1, int(best[3])), max(1, int(best[4]))

    def choose_square_grid_shape(self, width, height, entries):
        slot_count = max(1, sum(units for _widget, units in entries))
        width = max(1, int(width))
        height = max(1, int(height))
        best = None
        max_columns = min(slot_count, self.max_layout_columns or slot_count)
        for columns in range(1, max_columns + 1):
            rows = self.layout_rows_for_entries(entries, columns)
            if self.max_layout_rows is not None and rows > self.max_layout_rows:
                continue
            edge = min(width / columns, height / rows)
            empty_slots = (columns * rows) - slot_count
            used_area = edge * edge * slot_count
            score = used_area - (empty_slots * edge * 0.08)
            if best is None or score > best[0]:
                best = (score, columns, rows, edge)
        if best is None:
            columns = min(max_columns, slot_count)
            rows = self.layout_rows_for_entries(entries, columns)
            return columns, rows, max(1, int(min(width / columns, height / max(1, rows))))
        return best[1], best[2], max(1, int(best[3]))

    def fit_square_grid_layout(self, width, height):
    ...
    def fit_hexagonal_layout(self, width, height):
        ...
        for tile in self.tiles:
            ...
            tile.set_triangle_orientation(None)
        entries = self.layout_entries()
        columns, rows, tile_width, tile_height = self.choose_hexagon_shape(width, height, entries)
        self.current_columns = columns
        self.current_rows = rows
        ...
        for widget, index, span in self.iter_layout_positions(entries, columns):
            row = index // columns
            column = index % columns
            x = int(round(column * self.tile_width * 0.75))
            y = int(round(row * self.tile_height + (self.tile_height * 0.5 if column % 2 else 0)))
            if isinstance(widget, SimpleMirrorTile):
                widget.set_triangle_orientation("hex")
                widget.set_layout_size(self.tile_width, self.tile_height)
                self.grid.move(widget, x, y)
                widget.show()
                continue
            width_px = self.tile_width * (1.0 + max(0, span - 1) * 0.75)
    ...
    def choose_equilateral_triangle_shape(self, width, height, entries):
        if not entries:
            return 1, 1, max(1, int(width)), max(1, int(height))
        width = max(1, int(width))
        height = max(1, int(height))
        slot_count = sum(self.triangle_entry_slot_units(widget, units, "vertical" if height > width else "horizontal") for widget, units in entries)
        max_cells = max(1, slot_count)
        min_cells = max(1, max(self.triangle_entry_slot_units(widget, units, "vertical" if height > width else "horizontal") for widget, units in entries))
        max_columns = min(max_cells, self.max_layout_columns or max_cells)
        best = None
        flow = "vertical" if height > width else "horizontal"
        for cell_columns in range(min_cells, max_columns + 1):
            rows = self.triangle_rows_for_entries(entries, cell_columns, flow)
            if self.max_layout_rows is not None and rows > self.max_layout_rows:
                continue
            side = min(width / (1.0 + max(0, cell_columns - 1) * 0.5), height / (rows * EQUILATERAL_TRIANGLE_RATIO))
            tile_width = side
            tile_height = side * EQUILATERAL_TRIANGLE_RATIO
            empty_slots = (cell_columns * rows) - slot_count
            used_area = tile_width * tile_height * 0.5 * slot_count
            score = used_area - (empty_slots * 18)
            if best is None or score > best[0]:
                best = (score, flow, cell_columns, rows, tile_width, tile_height)
        if best is None:
            return "horizontal", 1, slot_count, max(1, int(width)), max(1, int(width * EQUILATERAL_TRIANGLE_RATIO))
        return best[1], best[2], best[3], max(1, int(best[4])), max(1, int(best[5]))

    def fit_equilateral_triangle_layout(self, width, height):
        visible_tiles = self.visible_tiles()
        for tile in self.tiles:
            tile.set_visible(tile in visible_tiles)
            tile.set_triangle_orientation(None)
        entries = self.layout_entries()
        flow, columns, rows, tile_width, tile_height = self.choose_equilateral_triangle_shape(width, height, entries)
        self.triangle_flow = flow
        self.current_columns = columns
        self.current_rows = rows
        self.tile_width = clamp(tile_width, MIN_TILE_WIDTH, MAX_TILE_WIDTH, self.tile_width)
        self.tile_height = clamp(tile_height, MIN_TILE_HEIGHT, MAX_TILE_HEIGHT, self.tile_height)
        for widget, cursor, span_slots in self.iter_triangle_layout_positions(entries, columns, flow):
            row = cursor // columns
            column = cursor % columns
    ...
    def fit_scalene_triangle_layout(self, width, height):
        visible_tiles = self.visible_tiles()
        for tile in self.tiles:
            tile.set_visible(tile in visible_tiles)
            tile.set_triangle_orientation(None)
        entries = self.layout_entries()
        flow, columns, rows, tile_width, tile_height = self.choose_triangle_flow_shape(width, height, entries)
        self.triangle_flow = flow
        self.current_columns = columns
        self.current_rows = rows
        self.tile_width = clamp(tile_width, MIN_TILE_WIDTH, MAX_TILE_WIDTH, self.tile_width)
        self.tile_height = clamp(tile_height, MIN_TILE_HEIGHT, MAX_TILE_HEIGHT, self.tile_height)
        for widget, cursor, span_slots in self.iter_triangle_layout_positions(entries, columns, "vertical" if height > width else "horizontal"):
            row = cursor // columns
            column = cursor % columns
            x = int(round(column * self.tile_width * 0.5))
            y = int(round(row * self.tile_height))
            if isinstance(widget, SimpleMirrorTile):
                widget.set_triangle_orientation("scalene-a" if (row + column) % 2 == 0 else "scalene-b")
                widget.set_layout_size(self.tile_width, self.tile_height)
                self.grid.move(widget, x, y)
                widget.show()
                continue
    ...
    def right_triangle_cell_count(self, entries):
        tile_count = sum(1 for widget, _units in entries if isinstance(widget, SimpleMirrorTile))
        utility_cells = sum(max(1, int(units)) for widget, units in entries if not isinstance(widget, SimpleMirrorTile))
        return max(1, int(math.ceil(tile_count / 2.0)) + utility_cells)

    def iter_right_triangle_layout_positions(self, entries, columns):
        columns = max(1, int(columns))
        tile_index = 0
        utility_cursor = None
        for widget, units in entries:
            if isinstance(widget, SimpleMirrorTile):
                cell_index = tile_index // 2
                half = tile_index % 2
                tile_index += 1
                yield widget, cell_index, 1, half
                continue
            if utility_cursor is None:
                utility_cursor = int(math.ceil(tile_index / 2.0))
            span = max(1, int(units))
            column = utility_cursor % columns
            if column + span > columns:
                utility_cursor += columns - column
            yield widget, utility_cursor, span, None
            utility_cursor += span

    def right_triangle_rows_for_entries(self, entries, columns):
        rows = 1
        for _widget, cell_index, span, _half in self.iter_right_triangle_layout_positions(entries, columns):
            end_cell = cell_index + max(1, int(span)) - 1
            rows = max(rows, end_cell // max(1, int(columns)) + 1)
        return rows

    def choose_right_triangle_shape(self, width, height, entries):
        cell_count = self.right_triangle_cell_count(entries)
        width = max(1, int(width))
        height = max(1, int(height))
        best = None
        max_columns = min(cell_count, self.max_layout_columns or cell_count)
        for columns in range(1, max_columns + 1):
            rows = self.right_triangle_rows_for_entries(entries, columns)
            if self.max_layout_rows is not None and rows > self.max_layout_rows:
                continue
            edge = min(width / columns, height / rows)
            empty_cells = (columns * rows) - cell_count
            score = edge * edge * cell_count - (empty_cells * edge * 0.08)
            if best is None or score > best[0]:
                best = (score, columns, rows, edge)
        if best is None:
            columns = min(max_columns, cell_count)
            rows = self.right_triangle_rows_for_entries(entries, columns)
            return columns, rows, max(1, int(min(width / columns, height / max(1, rows))))
        return best[1], best[2], max(1, int(best[3]))

    def fit_right_triangle_layout(self, width, height):
        visible_tiles = self.visible_tiles()
        for tile in self.tiles:
            tile.set_visible(tile in visible_tiles)
            tile.set_triangle_orientation(None)
        entries = self.layout_entries()
        columns, rows, edge = self.choose_right_triangle_shape(width, height, entries)
        self.current_columns = columns
        self.current_rows = rows
        self.tile_width = clamp(edge, MIN_TILE_WIDTH, MAX_TILE_WIDTH, self.tile_width)
        self.tile_height = clamp(edge, MIN_TILE_HEIGHT, MAX_TILE_HEIGHT, self.tile_height)
        placements = list(self.iter_right_triangle_layout_positions(entries, columns))
        cell_tile_counts = {}
        for widget, cell_index, _span, _half in placements:
            if isinstance(widget, SimpleMirrorTile):
                cell_tile_counts[cell_index] = cell_tile_counts.get(cell_index, 0) + 1

        for widget, cell_index, span, half in placements:
            row = cell_index // columns
            column = cell_index % columns
            x = int(round(column * self.tile_width))
            y = int(round(row * self.tile_height))
            if isinstance(widget, SimpleMirrorTile):
                widget.set_triangle_orientation(None if cell_tile_counts.get(cell_index, 0) < 2 else ("right-nw" if half == 0 else "right-se"))
                widget.set_layout_size(self.tile_width, self.tile_height)
                self.grid.move(widget, x, y)
                widget.show()
                continue
    ...
    def relayout_tiles(self, columns, rows):
        if self.mirror_layout_mode in {"blobs", "equilateral-triangles", "hexagons", "right-triangles", "scalene-triangles", "square-grid", "triangles", "voronoi"}:
            ...
            elif self.mirror_layout_mode == "right-triangles":
                self.fit_right_triangle_layout(max(1, alloc.width), max(1, alloc.height))
            elif self.mirror_layout_mode == "scalene-triangles":
    ...
    def fit_tiles_to_window(self):
        ...
        if self.mirror_layout_mode in {"blobs", "equilateral-triangles", "hexagons", "right-triangles", "scalene-triangles", "square-grid", "triangles", "voronoi"}:
            if self.mirror_layout_mode == "blobs":
                ...
                self.fit_right_triangle_layout(width, height)
            elif self.mirror_layout_mode == "scalene-triangles":
    ...
    def layout_vertical_delta(self):
        if self.mirror_layout_mode == "triangles" and self.triangle_flow == "vertical":
            return max(1, self.current_rows)
        return max(1, self.current_columns)
    ...
    def tile_at_panel_point(self, x, y):
        for tile in self.visible_tiles():
            if self.widget_contains_panel_point(tile, x, y):
                translated = tile.translate_coordinates(self.win, 0, 0)
                if translated is not None:
                    tile_x, tile_y = translated
                    alloc = tile.get_allocation()
                    if not tile.point_in_triangle(x - tile_x, y - tile_y, alloc.width, alloc.height):
                        continue
                return tile
        return None
    ...
    def show_context_menu(self, event, tile):
        ...
        self.add_check_item(menu, "Bordes de ventana", self.window_decorated, self.set_window_decorated)
        ...
        clock_menu.append(Gtk.SeparatorMenuItem())
        smaller_clock_item = Gtk.MenuItem(label="Fuente mas pequena")
        smaller_clock_item.connect("activate", lambda *_args: self.adjust_clock_font_size(-1))
        clock_menu.append(smaller_clock_item)
        larger_clock_item = Gtk.MenuItem(label="Fuente mas grande")
        larger_clock_item.connect("activate", lambda *_args: self.adjust_clock_font_size(1))
        clock_menu.append(larger_clock_item)
        ...
        self.add_check_item(menu, "Mostrar bandeja del sistema", self.show_tint2, self.set_show_tint2)
        self.add_check_item(menu, "Mostrar launchers", self.show_launchers, self.set_show_launchers)
        ...
        for label, mode in (
            ...
            if layout_group is None:
                layout_group = item
            item.set_active(self.mirror_layout_mode == mode)
            item.connect("toggled", lambda check, value=mode: check.get_active() and self.set_mirror_layout_mode(value))
            layout_menu.append(item)
        layout_menu.append(Gtk.SeparatorMenuItem())
        columns_label = "Columnas max: sin limite" if self.max_layout_columns is None else f"Columnas max: {self.max_layout_columns}"
        rows_label = "Filas max: sin limite" if self.max_layout_rows is None else f"Filas max: {self.max_layout_rows}"
        columns_item = Gtk.MenuItem(label=columns_label)
        columns_item.connect("activate", lambda *_args: self.choose_layout_limit("columns"))
        layout_menu.append(columns_item)
        rows_item = Gtk.MenuItem(label=rows_label)
        rows_item.connect("activate", lambda *_args: self.choose_layout_limit("rows"))
        layout_menu.append(rows_item)
        clear_limits_item = Gtk.MenuItem(label="Quitar limites")
        clear_limits_item.connect("activate", lambda *_args: self.set_layout_limits(None, None))
        layout_menu.append(clear_limits_item)
        layout_item = Gtk.MenuItem(label="Forma de mirrors")
    ...
    def set_mirror_layout_mode(self, mode):
        self.mirror_layout_mode = normalize_mirror_layout_mode(mode)
        for tile in self.tiles:
            tile.set_triangle_orientation(None)
        self.hovered_tile = None
        self.current_columns = 0
    ...
    def cycle_mirror_layout_mode(self, direction=1):
        try:
            index = MIRROR_LAYOUT_CYCLE.index(self.mirror_layout_mode)
        except ValueError:
            index = 0
        next_mode = MIRROR_LAYOUT_CYCLE[(index + int(direction)) % len(MIRROR_LAYOUT_CYCLE)]
        self.set_mirror_layout_mode(next_mode)

    def set_layout_limits(self, max_columns=None, max_rows=None):
        self.max_layout_columns = normalize_layout_limit(max_columns)
        self.max_layout_rows = normalize_layout_limit(max_rows)
        self.current_columns = 0
        self.current_rows = 0
        self.fit_tiles_to_window()
        self.save_state()

    def choose_layout_limit(self, axis):
        is_columns = axis == "columns"
        current = self.max_layout_columns if is_columns else self.max_layout_rows
        dialog = Gtk.Dialog(
            title="Limite de columnas" if is_columns else "Limite de filas",
            transient_for=self.win,
            flags=0,
        )
        dialog.add_button("Sin limite", Gtk.ResponseType.NO)
        dialog.add_button("Cancelar", Gtk.ResponseType.CANCEL)
        dialog.add_button("Guardar", Gtk.ResponseType.OK)
        box = dialog.get_content_area()
        adjustment = Gtk.Adjustment(value=current or 4, lower=1, upper=MAX_LAYOUT_LIMIT, step_increment=1, page_increment=4)
        spin = Gtk.SpinButton(adjustment=adjustment, climb_rate=1, digits=0)
        spin.set_numeric(True)
        box.pack_start(spin, False, False, 8)
        dialog.show_all()
        response = dialog.run()
        if response == Gtk.ResponseType.OK:
            if is_columns:
                self.set_layout_limits(spin.get_value_as_int(), self.max_layout_rows)
            else:
                self.set_layout_limits(self.max_layout_columns, spin.get_value_as_int())
        elif response == Gtk.ResponseType.NO:
            if is_columns:
                self.set_layout_limits(None, self.max_layout_rows)
            else:
                self.set_layout_limits(self.max_layout_columns, None)
        dialog.destroy()

    def set_window_decorated(self, active):
        self.window_decorated = bool(active)
        self.win.set_decorated(self.window_decorated)
        self.save_state()
    ...
    def set_clock_font_size(self, size):
        self.clock_font_size = clamp(size, MIN_CLOCK_FONT_SIZE, MAX_CLOCK_FONT_SIZE, DEFAULT_CLOCK_FONT_SIZE)
        self.clock_slot.set_font_size(self.clock_font_size)
        self.save_state()
    ...
    def set_show_launchers(self, active):
        self.show_launchers = bool(active)
        self.launcher_grid.set_visible(self.show_launchers)
        if self.show_launchers:
            self.launcher_grid.reload_launchers()
        self.current_columns = 0
        self.current_rows = 0
        self.fit_tiles_to_window()
        self.save_state()
    ...
    def on_key_press(self, _win, event):
        ...
        if key in ("q", "Q") and (event.state & Gdk.ModifierType.CONTROL_MASK):
            Gtk.main_quit()
            return True
        if key in ("m", "M") and (event.state & Gdk.ModifierType.CONTROL_MASK):
            direction = -1 if (event.state & Gdk.ModifierType.SHIFT_MASK) else 1
            self.cycle_mirror_layout_mode(direction)
            return True
        return False
